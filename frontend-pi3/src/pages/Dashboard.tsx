...
function Dashboard({ onLogout }: DashboardProps) {
  ...
  const [filtroEstante, setFiltroEstante] = useState<string>('TODAS');
  ...
  const estantesUnicas = Array.from(new Set(locais.map((l) => l.nome).filter((l) => l && l !== '')));

  const navegar = (pagina: string) => setTelaAtual(pagina as any);

  const produtosFiltrados = produtos.filter((p) => {
    if (filtroAtivo === 'BAIXO_ESTOQUE' && p.quantidade_atual >= 5) return false;
    if (filtroAtivo !== 'TODOS' && filtroAtivo !== 'BAIXO_ESTOQUE' && p.categoria !== filtroAtivo) return false;

    if (filtroEstante === 'TODAS') return true;
    if (filtroEstante === 'SEM_ESTANTE') return !p.nome_local || p.nome_local === '';
    return p.nome_local === filtroEstante;
  });

  //Rotas tela com Header
  if (telaAtual === 'entrada') return <>
    <Header paginaAtual="entrada" onNavigate={navegar} onLogout={onLogout} />
    <Entrada onVoltar={() => setTelaAtual('lista')} onNavigate={navegar} />
  </>;
  if (telaAtual === 'locais') return <>
    <Header paginaAtual="locais" onNavigate={navegar} onLogout={onLogout} />
    <Locais onVoltar={() => setTelaAtual('lista')} />
  </>;
  if (telaAtual === 'saida') return <>
    <Header paginaAtual="saida" onNavigate={navegar} onLogout={onLogout} />
    <Saida onVoltar={() => setTelaAtual('lista')} onNavigate={navegar} />
  </>;
  if (telaAtual === 'historico') return <>
    <Header paginaAtual="historico" onNavigate={navegar} onLogout={onLogout} />
    <Historico onVoltar={() => setTelaAtual('lista')} />
  </>;
  if (telaAtual === 'fornecedores') return <>
    <Header paginaAtual="fornecedores" onNavigate={navegar} onLogout={onLogout} />
    <Fornecedores onVoltar={() => setTelaAtual('lista')} />
  </>;

  return (
    <>
      <Header paginaAtual="lista" onNavigate={navegar} onLogout={onLogout} />
      <div className="dashboard-container" style={{ padding: 'clamp(16px, 4vw, 40px)', fontFamily: 'system-ui, -apple-system, sans-serif', maxWidth: '1300px', margin: '0 auto', backgroundColor: theme.background, minHeight: '100vh' }}>
      ...
      {/*Filtros*/}
      <div className="dashboard-filters" style={{ display: 'flex', gap: '12px', marginBottom: '18px', borderBottom: '1px solid #e7e5df', paddingBottom: '12px', overflowX: 'auto', flexWrap: 'wrap' }}>
        ...
        {categoriasUnicas.map(cat => (
          <button key={cat} title={cat} onClick={() => setFiltroAtivo(cat)} style={{ padding: '8px 12px', border: 'none', borderBottom: filtroAtivo === cat ? '3px solid ' + theme.primary : '3px solid transparent', cursor: 'pointer', fontWeight: 'bold', fontSize: '13px', backgroundColor: 'transparent', color: filtroAtivo === cat ? '#1c1917' : '#a8a29e', transition: 'all 0.2s', whiteSpace: 'nowrap' }}>{cat}</button>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: '12px', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', marginBottom: '24px' }}>
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
          <label style={{ fontSize: '13px', fontWeight: 700, color: '#57534e' }}>Estante</label>
          <select
            value={filtroEstante}
          ...
          >
            ...
            <option value="SEM_ESTANTE">Sem estante</option>
            {estantesUnicas.map((e) => (
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button
            ...
            style={{
              padding: '8px 12px',
              borderRadius: '8px',
              border: `1px solid ${modoVisualizacao === 'cards' ? theme.primary : '#e7e5df'}`,
              backgroundColor: modoVisualizacao === 'cards' ? theme.primaryLight : 'white',
              color: modoVisualizacao === 'cards' ? theme.primaryDark : '#57534e',
              fontWeight: 700,
              fontSize: '13px',
              cursor: 'pointer'
            }}
          ...
          <button
            ...
            style={{
              padding: '8px 12px',
              borderRadius: '8px',
              border: `1px solid ${modoVisualizacao === 'lista' ? theme.primary : '#e7e5df'}`,
              backgroundColor: modoVisualizacao === 'lista' ? theme.primaryLight : 'white',
              color: modoVisualizacao === 'lista' ? theme.primaryDark : '#57534e',
              fontWeight: 700,
              fontSize: '13px',
              cursor: 'pointer'
            }}
            ...
            produtosFiltrados.map((p) => (
              <div key={p.id} className="dashboard-list-item">
                <div className="dashboard-list-main">
                  <div>
                    <div className="dashboard-list-title">{p.nome}</div>
                    <div className="dashboard-list-meta">
                      {p.categoria} • {p.nome_local || 'Área Geral'}
                      {p.prateleira && p.prateleira !== 'Não informada' ? ` • ${p.prateleira}` : ''}
                    </div>
                  </div>
                  <div className="dashboard-list-qty">
                    {p.quantidade_atual} un.
                  </div>
                </div>
                {p.nome_fornecedor && (
                  <div className="dashboard-list-footer">Fornecedor: {p.nome_fornecedor}</div>
                )}
              </div>
            ))
          ) : (
            <div style={{ padding: '48px 20px', textAlign: 'center', color: '#a8a29e', fontSize: '16px', backgroundColor: 'transparent', border: '2px dashed #d6d3d1', borderRadius: '16px', fontWeight: '500' }}>
              Nenhum item foi encontrado nesta categoria.
            </div>
          )}
        </div>
